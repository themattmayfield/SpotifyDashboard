...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { topArtists, recentlyPlayed, topTracksShort, topArtistsShort } =
        await getUserInfo();
  ...
  return (
    <>
      <Layout profile>
        {topArtists && topTracksShort && topArtistsShort && recentlyPlayed ? (
          <>
            <div className="flex bg-transparent md:pr-6 md:pl-2 h-full">
              <div className="flex flex-col overflow-x-hidden md:mr-4 overflow-y-scroll mb-[100px] no-scrollbar h-full">
