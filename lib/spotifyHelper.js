...
export let REDIRECT_URI = process.env.NODE_ENV !== 'production' ? 'http://localhost:3000/api/callback/' : process.env.REDIRECT_URI
export let FRONTEND_URI = process.env.NODE_ENV !== 'production' ? 'http://localhost:3000' : 'https://spotify-dashboard-bay.vercel.app/'

export const LOGIN_URI = process.env.NODE_ENV !== 'production' ? 'http://localhost:3000/api/login' : process.env.LOGIN_URI
// TOKENS ******************************************************************************************
const EXPIRATION_TIME = 3600 * 1000; // 3600 seconds * 1000 = 1 hour in milliseconds

const setTokenTimestamp = () => process.browser && window.localStorage.setItem('spotify_token_timestamp', Date.now());
const setLocalAccessToken = token => {
  setTokenTimestamp();
  process.browser && window.localStorage.setItem('spotify_access_token', token);
};
const setLocalRefreshToken = token => process.browser && window.localStorage.setItem('spotify_refresh_token', token);
const getTokenTimestamp = () => process.browser && window.localStorage.getItem('spotify_token_timestamp');
const getLocalAccessToken = () => process.browser && window.localStorage.getItem('spotify_access_token');
const getLocalRefreshToken = () => process.browser && window.localStorage.getItem('spotify_refresh_token');
...
export const logout = () => {
  process.browser && window.localStorage.removeItem('spotify_token_timestamp');
  process.browser && window.localStorage.removeItem('spotify_access_token');
  process.browser && window.localStorage.removeItem('spotify_refresh_token');
  process.browser && (window.location.href = 'http://localhost:3000');
  // process.browser && window.location.reload();
};
...
export const getUserInfo = () =>
  axios
    .all([getUser(), getPlaylists(), getTopArtistsLong(), getTopTracksLong(), getRecentlyPlayed(), getTopTracksShort(), getTopArtistsShort()])
    .then(
      axios.spread((user, playlists, topArtists, topTracks, recentlyPlayed, topTracksShort, topArtistsShort) => ({
        user: user.data,        
