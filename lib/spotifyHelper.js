...
export let FRONTEND_URI =
  process.env.NODE_ENV !== "production"
    ? "http://localhost:3000"
    : "https://spotify-dashboard-bay.vercel.app/";
...
const getTokenTimestamp = () =>
  process.browser && window.localStorage.getItem("spotify_token_timestamp");
const getLocalAccessToken = () =>
  process.browser && window.localStorage.getItem("spotify_access_token");
const getLocalRefreshToken = () =>
  process.browser && window.localStorage.getItem("spotify_refresh_token");

// Refresh the token
const refreshAccessToken = async () => {
  try {
    const { data } = await axios.get(
      `${FRONTEND_URI}api/refresh_token?refresh_token=${getLocalRefreshToken()}`
    );
 ...
 */
export const followArtist = (artistId, method) => {
  const url = `https://api.spotify.com/v1/me/following?type=artist&ids=${artistId}`;
  return axios({ method: method, url, headers });
};
